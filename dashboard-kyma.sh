...
kubectl exec -ti -n $tenant $influxdb_pod -c influxdb -- influx -execute 'CREATE DATABASE jmeter'
...
kubectl exec -ti -n $tenant $master_pod -c jmmaster -- cp -r /load_test /jmeter/load_test

kubectl exec -ti -n $tenant $master_pod -c jmmaster -- chmod 755 /jmeter/load_test

##kubectl cp $working_dir/influxdb-jmeter-datasource.json -n $tenant $grafana_pod:/influxdb-jmeter-datasource.json

kubectl exec -ti -n $tenant $grafana_pod -c grafana -- curl 'http://admin:admin@127.0.0.1:3000/api/datasources' -X POST -H 'Content-Type: application/json;charset=UTF-8' --data-binary '{"name":"jmeterdb","type":"influxdb","url":"http://jmeter-influxdb:8086","access":"proxy","isDefault":true,"database":"jmeter","user":"admin","password":"admin"}'
